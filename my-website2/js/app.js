...
const OMDb_CONFIG = {
  KEY: 'http://www.omdbapi.com/?i=tt3896198&apikey=ea0e5068', // ← REPLACE THIS WITH YOUR ACTUAL OMDb KEY
...
/**
 * Fetches movies by genre with loading state
 */
async function fetchMoviesByGenre(genreId, genreName) {
  ...
  try {
    ...
    const response = await fetch(
      `${API_CONFIG.BASE_URL}/discover/movie?api_key=${API_CONFIG.KEY}&with_genres=${genreId}&sort_by=popularity.desc&page=1`
    );
    
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    
    const data = await response.json();
    displayGenreResults(data.results, genreName, 'movie');
    closeSearchModal();
...
/**
 * Fetches IMDb rating from OMDb (simple enhancement)
 */
async function fetchIMDbRating(movieTitle, movieYear) {
  // Skip if no OMDb key
  if (!OMDb_CONFIG.KEY || OMDb_CONFIG.KEY === 'your_omdb_key_here') {
    return null;
  }
  
  try {
    const response = await fetch(
      `${OMDb_CONFIG.BASE_URL}?apikey=${OMDb_CONFIG.KEY}&t=${encodeURIComponent(movieTitle)}&y=${movieYear || ''}`
    );
    ...
    console.log('OMDb not available:', error.message);
    return null;
  }
}
 ...
 */
async function fetchTVByGenre(genreId, genreName) {
  try {
    showLoading();
    const response = await fetch(
      `${API_CONFIG.BASE_URL}/discover/tv?api_key=${API_CONFIG.KEY}&with_genres=${genreId}&sort_by=popularity.desc&page=1`
    );
    ...
    // Create a special genre section
    displayGenreResults(data.results, genreName, 'tv');
    closeSearchModal();
    
  } catch (error) {
    console.error('Error fetching genre TV shows:', error);
    showError(`Failed to load ${genreName} TV shows`);
  } finally {
    hideLoading();
  }
}
...
/**
 * Displays genre results in a new section
 */
function displayGenreResults(items, genreName, type) {
  // Remove any existing genre section
  const existingGenreSection = document.getElementById('genre-results-section');
  if (existingGenreSection) {
    existingGenreSection.remove();
  }
  
  // Create new genre section
  const genreSection = document.createElement('section');
  genreSection.id = 'genre-results-section';
  genreSection.className = 'content-section genre-results-section';
  
  const typeText = type === 'movie' ? 'Movies' : 'TV Shows';
  ...
  genreSection.innerHTML = `
    <div class="section-header">
      <h2 class="section-title">
        ...
        ${genreName} ${typeText}
        <span class="grid-count">(${items.length} items)</span>
      </h2>
      <button class="btn btn-outline" onclick="closeGenreSection()">
        <i class="fas fa-arrow-left"></i> Back to Browse
      </button>
    </div>
    <div class="trending-movies-grid" id="genre-results-grid">
      <!-- Genre results will be loaded here -->
    </div>
  `;
  
  // Insert after the trending section
  const trendingSection = document.getElementById('trending-section');
  trendingSection.parentNode.insertBefore(genreSection, trendingSection.nextSibling);
  
  // Display the results
  const container = document.getElementById('genre-results-grid');
  if (items && items.length > 0) {
    ...
    container.innerHTML = `
      <div class="empty-section" style="grid-column: 1 / -1; text-align: center; padding: 40px; color: var(--text-muted);">
        ...
        <p>No ${genreName} ${typeText.toLowerCase()} found.</p>
      </div>
    `;
  }
}
...
/**
 * Closes the genre results section
 */
function closeGenreSection() {
  const genreSection = document.getElementById('genre-results-section');
  if (genreSection) {
    genreSection.remove();
  }
}
...
/**
 * Enhanced search function with genre suggestions
 */
async function searchTMDB() {
  ...
  if (!query) {
    // Show genre suggestions when search is empty
    displayGenreSuggestions();
    return;
  }
  ...
  try {
      ...
      container.innerHTML = `
        <div class="empty-state">
          ...
          <div class="search-suggestions">
            <p>Try browsing by genre instead:</p>
            <button class="btn btn-outline" onclick="displayGenreSuggestions()">
              <i class="fas fa-tags"></i> Browse Genres
            </button>
          </div>
        </div>
      `;
    ...
    container.innerHTML = `
      <div class="empty-state">
        ...
        <button class="btn btn-outline" onclick="displayGenreSuggestions()">
          <i class="fas fa-tags"></i> Browse Genres Instead
        </button>
      </div>
    `;
  }
...
document.getElementById('modal-date').textContent = 
  formatDate(item.release_date || item.first_air_date);
    ...
    // ADD IMDb rating (in background)
  if (item.title || item.name) {
    fetchIMDbRating(item.title || item.name, releaseYear)
      .then(imdbData => {
        if (imdbData) {
          const ratingContainer = document.getElementById('modal-rating-text');
          if (ratingContainer) {
            const imdbElement = document.createElement('div');
            imdbElement.className = 'imdb-rating';
            imdbElement.innerHTML = `
              <div class="imdb-badge">
                <span class="imdb-logo">IMDb</span>
                <span class="imdb-score">${imdbData.rating}/10</span>
              </div>
            `;
            ratingContainer.appendChild(imdbElement);
          }
        }
      })
      .catch(error => {
...
/**
 * Opens search modal and shows genre suggestions
 */
function openSearchModal() {
  ...
  // Show genre suggestions when opening search
  displayGenreSuggestions();
}
...
// Expose functions globally
  ...
  window.displayGenreSuggestions = displayGenreSuggestions;
  window.closeGenreSection = closeGenreSection;
  window.startWatchingBanner = startWatchingBanner;
   window.fetchIMDbRating = fetchIMDbRating;
}
